"""
Strategy: EMA Crossover Short_15m-1m_TSL_0
Generated: 2026-02-03 08:28:30.009270
Market: NSE:NIFTY50-INDEX
Timeframe: 15m-1m (MTF: Signal on 15m, Entry on 1m)
...
class EMA_Crossover_Short_15m_1m_TSL_0_Strategy:
    """
    EMA Crossover Short_15m-1m_TSL_0
    ...
    def __init__(self) -> None:
        ...
        self.timeframe = "15m-1m"
        self.risk_reward = 5.2
        self.ema_fast = 13
        self.ema_slow = 200
    ...
    def calculate_signal(self, df: pd.DataFrame) -> Optional[str]:
        ...
        vwap_val = ind['vwap'][idx] if self.use_vwap else None
        
        # Status "Thinking" logic
        trend_bearish = ema_f < ema_s
        below_emas = price < ema_f and price < ema_s
        below_vwap = True
        if self.use_vwap and vwap_val:
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        ind['vwap_upper'], ind['vwap_lower'] = vwap_bands(ind['vwap'], close, self.vwap_band_multiplier)
    ...
    def check_entry(self, open_: np.ndarray, close: np.ndarray, high: np.ndarray, low: np.ndarray, ind: Dict[str, np.ndarray], idx: int, timestamps=None) -> Optional[str]:
        ...
        try:
            ...
            if np.isnan(ema_fast) or np.isnan(ema_slow):
                return None
            # Rule: ema_fast_below_slow
            if ema_fast >= ema_slow:
                return None
            # Rule: price_below_both_emas
            if price >= ema_fast or price >= ema_slow:
                return None
            # Rule: price_below_vwap (with 0.15% tolerance)
            if self.use_vwap and 'vwap' in ind:
                vwap_val = ind['vwap'][idx]
                if not np.isnan(vwap_val):
                    # Allow 0.15% above VWAP to catch pullbacks
...
if __name__ == "__main__":
    ...
    print("")
    print(f"  2. strategy = EMA_Crossover_Short_15m_1m_TSL_0_Strategy()")
