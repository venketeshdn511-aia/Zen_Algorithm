# -*- coding: utf-8 -*-
"""
Strategy: EMA Crossover Long_5m-1m_TSL_786
Generated: 2026-02-04 21:07:42.069969
...
# ==============================================================================
# SHARED BACKTEST LOGIC (Synced with Internal Engine)
# ==============================================================================

def check_exit_long(low, high, stop_loss, take_profit):
...
def check_exit_short(low, high, stop_loss, take_profit):
...
def trailing_stop_long(current_close, current_sl, trail_distance):
    """Update trailing stop for LONG position."""
    new_sl = current_close - trail_distance
    return new_sl if new_sl > current_sl else current_sl

def trailing_stop_short(current_close, current_sl, trail_distance):
    """Update trailing stop for SHORT position."""
    new_sl = current_close + trail_distance
    return new_sl if new_sl < current_sl else current_sl

def calculate_pnl(entry_price, exit_price, is_long):
    """Calculate PnL for a trade."""
    pnl = (exit_price - entry_price) if is_long else (entry_price - exit_price)
    pnl_percent = (pnl / entry_price) * 100.0 if entry_price != 0 else 0.0
...
def calculate_cagr(total_pnl, initial_capital, days):
    ...
    if ending_capital <= 0:
        return -100.0
    return ((ending_capital / initial_capital) ** (1.0 / years) - 1.0) * 100

def calculate_sharpe(daily_returns, risk_free_rate=0.05):
    ...
    if std_dev == 0:
        return 0.0
    return (np.mean(excess) / std_dev) * np.sqrt(252)

def calculate_sortino(daily_returns, risk_free_rate=0.05):
...
class EMA_Crossover_Long_5m_1m_TSL_786_Strategy:
    """
    EMA Crossover Long_5m-1m_TSL_786
    ...
    def check_entry(self, open_: np.ndarray, close: np.ndarray, high: np.ndarray, low: np.ndarray, ind: Dict[str, np.ndarray], idx: int, timestamps=None) -> Optional[str]:
        ...
        try:
            ...
            if np.isnan(ema_fast) or np.isnan(ema_slow):
                return None
            # Rule: ema_fast_above_slow
            if ema_fast <= ema_slow:
                return None
            # Rule: price_above_both_emas
    ...
    def check_exit(self, position: str, current_price: float, high_val: float, low_val: float, stop_loss: float, take_profit: float) -> Tuple[bool, float]:
        ...
        Uses shared functions for SYNC ACCURACY with Internal Engine.
        """
        if position == "LONG":
            return check_exit_long(low_val, high_val, stop_loss, take_profit)
        else:
            return check_exit_short(low_val, high_val, stop_loss, take_profit)
    ...
    def backtest(self, df, df_analysis=None):
        ...
        for i in range(start_idx, len(close)):
            ...
            if position is not None and self.trailing_sl:
                ...
                if position == "LONG":
                    stop_loss = trailing_stop_long(close[i], stop_loss, trail_dist)
                else:
                    stop_loss = trailing_stop_short(close[i], stop_loss, trail_dist)
...
if __name__ == "__main__":
    print(f"--- Strategy: EMA Crossover Long_5m-1m_TSL_786 ---")
    ...
    print("")
    print(f"  2. strategy = EMA_Crossover_Long_5m_1m_TSL_786_Strategy()")
    ...
    print("  6. initial_capital = 100000.0")
    print("  7. days = 365 # Approx")
    print("")
    print("  # INSTITUTIONAL METRICS")
    print("  print(f'Total PnL: {total_pnl:.2f}')")
    print("  print(f'CAGR: {calculate_cagr(total_pnl, initial_capital, days):.2f}%')")
    print("  # Calculate Daily Returns for Sharpe")
    print("  # Note: Requires trade timestamps or aggregating PnL by date")
    print("  print(f'Max Losing Streak: {calculate_max_consecutive_losses(closed_trades)}')")
