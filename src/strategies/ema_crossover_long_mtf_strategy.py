...
# -*- coding: utf-8 -*-
"""
Strategy: EMA Crossover Long_15m-1m_191
Generated: 2026-02-04 06:18:36.283719
...
class EMA_Crossover_Long_15m_1m_191_Strategy:
    """
    EMA Crossover Long_15m-1m_191
    ...
    def calculate_signal(self, df: pd.DataFrame) -> Optional[str]:
        ...
        rsi_val = ind['rsi'][idx] if self.use_rsi else 50
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        ind['rsi'] = rsi(close, self.rsi_period)
    ...
    def check_entry(self, open_: np.ndarray, close: np.ndarray, high: np.ndarray, low: np.ndarray, ind: Dict[str, np.ndarray], idx: int, timestamps=None) -> Optional[str]:
        ...
        try:
            ...
            if price <= ema_fast or price <= ema_slow:
                return None
            # Rule: rsi_above_30
            if 'rsi' in ind and not np.isnan(ind['rsi'][idx]):
                if ind['rsi'][idx] <= 30:
                    return None
...
if __name__ == "__main__":
    print(f"--- Strategy: EMA Crossover Long_15m-1m_191 ---")
    ...
    print("")
    print(f"  2. strategy = EMA_Crossover_Long_15m_1m_191_Strategy()")
